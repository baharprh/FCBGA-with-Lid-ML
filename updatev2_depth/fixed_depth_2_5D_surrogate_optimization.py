...
- Fixed tree depths for final RF/XGB models (no automatic depth tuning)
...
- Final surrogate models use fixed max_depth values for consistency.
...
FILE_CONFIGS = [
    # Solder-joint reliability dataset
    {'name': '2D_SJR_lid_300_v4 (1).xlsx'},
    # Assembly stress/warpage dataset
    {'name': '2D_assembly_lid_300_v4.xlsx'},
    # Optional thermal dataset. Keep it here if you have it; the script skips it if missing.
...
# Fixed depths used by the final models.
# Validation curves are still plotted for diagnostics, but the final surrogate
# models always use these values.
...
def run_combined_workflow():
    ...
    print(f"Using fixed depths: RF={RF_FIXED_MAX_DEPTH}, XGB={XGB_FIXED_MAX_DEPTH}")
    ...
    for tname in initial_objective_names:
        ...
        # Fit both RF and XGB using fixed depths.
        # This makes the term-paper method easier to explain and keeps the
        # model architecture consistent across all objectives.
        ...
        rf_model = RandomForestRegressor(**rf_base_params, max_depth=RF_FIXED_MAX_DEPTH)
        xg_model = XGBRegressor(**xgb_base_params, max_depth=XGB_FIXED_MAX_DEPTH)
        ...
        # Diagnostics plot (2x3) for both model classes.
        # The validation curves still show how depth affects performance, but the
        # learning/parity plots use the fixed depth values below.
        for model_name, model_class, p_range, base_p, best_val in [
            ("RF", RandomForestRegressor, RF_MAX_DEPTH_RANGE, rf_base_params, RF_FIXED_MAX_DEPTH),
            ("XGB", XGBRegressor, XGB_MAX_DEPTH_RANGE, xgb_base_params, XGB_FIXED_MAX_DEPTH),
        ]:
